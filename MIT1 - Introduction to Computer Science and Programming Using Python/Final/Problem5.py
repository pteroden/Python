...
class edx(object):
    ...
    def setGrade(self, grade, course="6.01x"):
        ...
        for n in range(len(self.myCourses)):
            if self.myCourses[n].courseName == course:
                self.myCourses[n].setGrade(grade)
    ...
    def getGrade(self, course="6.02x"):
        ...
        for n in range(len(self.myCourses)):
            if self.myCourses[n].courseName == course:
                return self.myCourses[n].getGrade()
        return -1
    ...
    def setPset(self, pset, score, course="6.00x"):
        ...
        """
        assert type(pset) == str or int
        assert 100 >= score >= 0
        assert type(course) == str

        for n in range(len(self.myCourses)):
            if self.myCourses[n].courseName == course:
                self.myCourses[n].setPset(pset, score)
    ...
    def getPset(self, pset, course="6.00x"):
        ...
        for n in range(len(self.myCourses)):
            if self.myCourses[n].courseName == course:
                return self.myCourses[n].getPset(pset)
        return -1
