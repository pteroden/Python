...
def isValidWord2(word, hand):
    ...
    for letter in wordDic:
        if wordDic.get(letter, 0) > hand.get(letter, 0):
            return False
    return True
...
def compPlayHand(hand, wordList, n):
    ...
    while sum(hand.values()) > 0:
        ...
        if comp != None:
            wordscore = getWordScore(comp, n)
            score += wordscore
            hand = updateHand(hand, comp)
            print '\"%s\" earned %d points. Total: %d points' %(comp, wordscore, score)
            print
        else:
            break
...
def playGame(wordList):
    ...
    while True:
        ...
        if user == 'e':
            ...
            handcur = dealHand(HAND_SIZE)
            while True:
                user2 = raw_input('Enter u to have yourself play, c to have the computer play: ')
                if user2 == 'u':
                    playHand(handcur, wordList, HAND_SIZE)
                    break
                elif user2 == 'c':
                    compPlayHand(handcur, wordList, HAND_SIZE)
                    break
                else:
